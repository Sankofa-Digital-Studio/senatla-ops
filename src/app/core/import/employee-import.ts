...
export function stageEmployeeCsv(text: string, siteId: string, existing: Employee[] = []): EmployeeImportRow[] {
  ...
  const seenIds = new Set(existing.map((employee) => employee.idNumber));
  const seenCompanyNumbers = new Set(existing.map((employee) => employee.companyNumber?.toLowerCase()).filter(Boolean));
  return lines.slice(headerIndex + 1).map((line, index) => ({ values: splitCsv(line), sourceRow: headerIndex + index + 2 }))
    ...
    .map(({ values, sourceRow }) => {
      ...
      const idNumber = digits(raw['id_no'] || ''); const companyNumber = (raw['company_no'] || '').trim();
      const designation = (raw['designation'] || '').trim(); const startDate = normalizeDate(raw['emp_start_date'] || '');
      ...
      else if (seenCompanyNumbers.has(companyNumber.toLowerCase())) errors.push('Company number duplicates an existing or staged employee.');
      ...
      if (companyNumber) seenCompanyNumbers.add(companyNumber.toLowerCase());
      const additionalFields: Record<string, string> = {};
      headers.filter((header) => !knownHeaders.has(header) && raw[header]).forEach((header) => { additionalFields[header] = raw[header]; });
